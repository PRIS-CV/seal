...
class GuidedSelectiveLoss(Loss):
    ...
    def forward(self, logits, targets):
        ...
        BCE_can = self.alpha_can * targets_can * torch.log(torch.clamp(xs_neg, min=1e-8))
        # from IPython import embed; embed()
        # import ipdb; ipdb.sset_trace()

        BCE_loss = BCE_pos + BCE_neg + BCE_unl + BCE_can

        # Adding asymmetric gamma weights
        with torch.no_grad():
            asymmetric_w = torch.pow(1 - xs_pos * targets_pos - xs_neg * (targets_neg + targets_unl + targets_can),
                                     self.gamma_pos * targets_pos + self.gamma_neg * targets_neg +
                                     self.gamma_unl * targets_unl + self.gamma_can * targets_can)
