...
class VAWGraph:
    ...
    def __init__(self, f_graph_emb, f_adjacent_matrix, use_impress=True, weighted=False):
        super().__init__()
        
        self.f_graph_emb = f_graph_emb
        self.f_adjacent_matrix = f_adjacent_matrix
        self.weighted = weighted
        self.use_impress = use_impress

        assert op.exists(self.f_graph_emb), "Please Generate the graph embedding first."
        assert op.exists(self.f_adjacent_matrix), "Please Generate the adjacent matrix first."

        self.graph_emb = torch.cat(list(torch.load(self.f_graph_emb).values()), dim=0)
        self.adjacent_matrix = torch.load(self.f_adjacent_matrix)

        self.edge_index, self.edge_weight = self.encode_edges(self.adjacent_matrix, use_impress=use_impress, weighted=self.weighted)

    def encode_nodes(self):
        attrs = self.vaw_dataset.get_attr()
        return self.node_encoder(attrs, dir=self.vaw_dataset.anno_path)

    def encode_edges(self, adjacent_matrix, use_impress=True, weighted=False):
        
        edge_index = []
        edge_weight = []

        num = len(adjacent_matrix)
        for i in range(num):
            for j in range(num):
                if not use_impress:
                    if adjacent_matrix[i][j] > 0:
                        ...
                        if weighted:
                            edge_weight.append(adjacent_matrix[i][j])
                else:
                    if adjacent_matrix[i][j] != 0:
                        edge_index.append([i, j])
                    if weighted:
    ...
    def get_graph_emb(self):
        return self.graph_emb
    
    def node_dim(self):
        return self.graph_emb.shape[1]

    def get_edge_index(self):
        return torch.tensor(self.edge_index, dtype=torch.long).t().contiguous()

    def get_edge_weight(self):
        return torch.stack(self.edge_weight)


class AttributesGraph(object):
    
    def __init__(self, dataset, store=True) -> None:
        self.dataset = dataset
        self.store = store
        self.pos_corr_mat = np.identity(self.dataset.get_attr_num())
        self.neg_corr_mat = np.identity(self.dataset.get_attr_num())
        
    def _conditional_prob(self, ):
        pass
